...
namespace WasteManagement
{
    class Waste
    {
        ...
        public DataTable GetWasteList(string MonthNumber,int Year,int Days,bool ThisMonth)
        {
            ...
            string qry = $@"SELECT b.TPROD,idesc, date('20'||(substring((ttdte),1,2)-72)||'-'||substring(ttdte,3,2)||'-'||substring(ttdte,5,2)) as date  , sum(b.TQTY) as sum  ,day('20'||(substring((ttdte),1,2)-72)||'-'||substring(ttdte,3,2)||'-'||substring(ttdte,5,2)) as day,TTYPE
                          ...
                          GROUP BY b.TPROD,idesc, b.TTDTE,TTYPE                           UNION

                           SELECT  case  B.TTYPE when 'P1' then 'SC-CT350_' when 'P2' then 'SC-RD010_' when 'P3' then 'SC-CT375_' end TPROD,'' as idesc , date('20'||(substring((ttdte),1,2)-72)||'-'||substring(ttdte,3,2)||'-'||substring(ttdte,5,2)) as date,INT(SUM(TQTY*-ICSCP1)) as sum ,day('20'||(substring((ttdte),1,2)-72)||'-'||substring(ttdte,3,2)||'-'||substring(ttdte,5,2)) as day,TTYPE
                           FROM BPCSFV30.ITH  as b  inner join BPCSFV30.CICL01 as c on b.TPROD=c.ICPROD
                           WHERE TTDTE between {DateByS400Start} and {DateByS400End} and TTYPE in ('P1','P2','P3')
                           GROUP BY case  B.TTYPE when 'P1' then 'SC-CT350-actually' when 'P2' then 'SC-RD010' when 'P3' then 'SC-CT375' END,b.TTDTE,TTYPE 
                           ORDER BY TTYPE,date";
            ...
            string qryCatalog = $@"SELECT MSMKT15 as TPROD ,CASE trim(MSMKT15) when 'SC-CT350_' then 'Cured Tires Scraps-Actually' when 'SC-RD010_' then 'Rubberized Textile scrap R&D-Actually' when 'SC-CT375_' then 'prcs dev tire scrap-Actually' else idesc end idesc  
        ...
        private void DesignTable()
        {
            ...
            DtToShow.Columns.Add("Mtd Scrap% Warehouse");//אחוז פסולת כללית מתוך כלל הייצור
            ...
            for (int i = 0; i < DtToShow.Rows.Count; i++)//ממלא כל תיאור פריט בנפרד
            {
                ...
                {
                    if (DtToShow.Rows[i][0].ToString().TrimEnd() == DTgeneral.Rows[j]["TPROD"].ToString().TrimEnd())
                    {
        ...
        private void FillTargets()
        {
            ...
            for (int i = 0; i < DtToShow.Rows.Count; i++)
            {
                ...
                {
                    if (DtToShow.Rows[i]["Catalog Number"].ToString().Trim() == "SC-CT350_")//Cured Tires left overs        
                ...
                if (!string.IsNullOrEmpty(DtToShow.Rows[i]["Scrap TGT %"].ToString() as string) && EntryToWarehouse!=0 && SumProduction!=0)
                {
                    ...
                    DtToShow.Rows[i]["Mtd Scrap% Warehouse"] = ScrapPrecentOfWarehouse.ToString("p3");//2 מספרים אחרי הנקודה באחוזים
